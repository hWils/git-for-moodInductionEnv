...
beginningInstructions = "This experiment is equivalent to the practice experiment, except longer in duration and with breaks throughout. \n \n Please ask now if you have any questions about what you will do next. \n  \n As soon as you press a key, the experiment will immediately commence and your first visual stimuli will appear on the screen."
task = 'scary'
saveDataList = []
...
mon = monitors.Monitor('DellXPS', width = 35) #width = 42.926)#fetch the most recent calib for this monitor
...
# generates the csv file names each time taking into account participant number
def determineFileName():
    global filename
    filename ='VividnessData' + str(participantNumber) + '.csv'
...
def blockFunction():
    ...
    for filename in image_list:
        ...
        # creates a list to be written as a row for the csv file
        containerList = []
        containerList.append(stimulusName)
        containerList.append(intKey)
        containerList.append(trialNumber)
        containerList.append(block)
        writeToCSV(containerList)
        trialNumber +=1
    print(containerList)
    # then break
    breakWait()
...
def mainExperiment():
  ...
  #      win.flip()
    ...
    determineFileName()
    writeToCSV(['Image','Vividness', 'TrialNumber','Block', task])
